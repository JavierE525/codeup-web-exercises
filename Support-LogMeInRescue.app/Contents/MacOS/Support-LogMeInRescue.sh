...
# ===========================================================================
    ...
    if [[ "${Init}" == /*var/tmp* ]]; then
        "${Init}/Support-LogMeInRescue" --launch-- $@ &
        exit 0
    fi

    if [ -f "$1/Support-LogMeInRescue.app/Contents/MacOS/Support-LogMeInRescue" ]; then
        "$1/Support-LogMeInRescue.app/Contents/MacOS/Support-LogMeInRescue" --launch-- $@ &
        exit 0
    fi
    ...
    xattr -d "com.apple.quarantine" "${InstDir}/Support-LogMeInRescue.app"
